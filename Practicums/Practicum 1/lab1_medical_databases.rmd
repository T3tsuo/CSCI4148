...
```         
PatientID: Unique Identifier that a Patient is assigned to when registered to said database
AppointmentID: Unique Identifier that a Patient creates when setting up an appointment to help identify which appointment it is.
Gender: Social Characteristic of a person
ScheduledDate: The time set for when the person schedules an appointment
AppointmentDate: The time of which the said appointment has been executed.
Age: The absolute value of the date of birth of the person substracted by the current time.
Neighbourhood: A district of Vitória where the appointment resides.
SocialWelfare: Person has registered support from Bolsa Família welfare payments when they meet specific requirements. If they drop out of school or miss too much of school then the payments can get suspended.
Hypertension: boolean if person has had hypertension or not
Diabetes: patient was previously diagnosed with diabetes
AlcoholUseDisorder: patient was previously diagnosed with alcohol use disorder
Disability: person was diagnosed with a disability on a scale of 0-4
SMSReceived: At least received one reminder boolean
NoShow: Patient did not show up to scheduled appointment (bool)
```

**2** Can you think of 3 hypotheses for why someone may be more likely to miss a medical appointment?

```         
Time conflict with their work.

Too occupied with their childrens extracurricular activitites.

Could be dead between the time they scheduled the appointment and the time of the appointment.
```
...
```         
What type of disabilities does the patient have. Could provide useful information about the type of condition the person is in.

Distance between the patients home to the appointment location.

The patients reason for scheduling an appointment to assess the severity of it (or estimation of how long the appointment will take).
```
...
```{r}
count(raw.data, PatientID, sort = TRUE)[1,2]
```
...
```         
ScheduledDate is the most correlated with No Show.
```

**8** Are there any other variables which strongly correlate with one another?

```         
Scheduled Date is the most correlated with SMSReceived
```

**9** Do you see any issues with PatientID/AppointmentID being included in this plot?

```         
PatientID does not have any correlations to AppointmentID while AppointmentID has correlation to PatientID.
```
...
```         
Towards the end of the 1st plot, the data is so minuscule when you get near age 110-120 so you would not be able to see the difference in Show/No show and having Plot 2 shows better ratio comparisons than the graph in plot 1. So here you can clearly see the difference. Same with with the difference between the younger ages not showing up. In plot 1 it looks like they don't show up most of the time but looking at plot 2 you can clearly see that's not the case.
```
...
```         
The plot could be wrong because there are so many more people who have SMSReceived than people who don't, so there is more data of people just not showing up, but yet overall more people show up with SMSReceived than not. If we the ratio of overall the amount of people that attended their appointments before SMSReceived was implemented and then compare it to the overall ratio of when it was implemented, there would be a higher percentage of people showing up to their appointments.
```
...
ggplot(raw.data) + 
  ...
  ggtitle("Density of Disability and No Show")
...
```         
Probably because the location of the appointments are pretty accessible for most neighbourhoods, but then for the further away neighbourhoods it may be more difficult with transportation (and if they have a disability that makes it much harder) to get there.
```
...
```         
The amount of people that schedule and show up on the same day is huge, but drops immediately. So most of the time people try to schedule the day of, and will attend as well, but after that it's near fifty-fifty but more towards not showing up.
```
...
```         
I think it should be at least 80% accurate due to the fact that there is a large subset of data we are training with and the data we are given seems decently accurate.
```
...
fit.control <- trainControl(method="repeatedcv",number=3,
                           classProbs = TRUE, summaryFunction = twoClassSummary)

caret.model <- train(NoShow ~ .,data=train, method="gbm",metric="ROC", trControl=fit.control)

caret.pred <- predict(caret.model, newdata=test)
caret.probs <- predict(caret.model, newdata=test, type="prob")

test <- test %>% mutate(NoShow.numerical = ifelse(NoShow=="Yes",1,0))
confusionMatrix(caret.pred, test$NoShow, positive="Yes")
paste("GBM Area under ROC Curve: ", round(auc(test$NoShow.numerical, caret.probs[,2]),3), sep="")
```

**18** Based on everything, do you think we can trust analyses based on this dataset? Explain your reasoning.

```         
Even though there were some bias with SMSReceived and the amount of NoShows there were. I still believe that it is reasonable since the ML training had good output (Accuracy1: 0.8003, Accuracy2: 0.80009) and also the analysis made sense logically.
```
