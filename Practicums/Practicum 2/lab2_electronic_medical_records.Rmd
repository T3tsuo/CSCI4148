...
```         
note_id: Unique identifier for a specific note documented
description: Description of the person and/or medical issue they have.
medical_specialty: The specific medical category the condition belongs to.
sample_name: The procedure to help solve the issue.
transcription: More structured version of description (like notes)
keywords: Key names of persons issues
```
...
```{r}
tokenized.data.sentences.unigram <- analysis.data %>% tidytext::unnest_tokens(sentences, token="sentences", transcription, to_lower=TRUE)

tokenized.data.sentences.unigram %>% dplyr::group_by(medical_specialty) %>% dplyr::distinct(sentences) %>% dplyr::summarise(n=dplyr::n())
...
```         
It might not be great because if it is a general purpose, maybe it will take into context a word from a non medical point of view and group it with other wrong terms.
```
...
```         
There is a lot more data with surgery, but the spread with surgery correlation-wise is large. There is more data that is not correlated but also some that are more correlated to Orthopedic than Radiology has.
```
...
Dissection Carried. Because it appears in both surgery and orthopedic, but more of them are in surgery.
...
Tthe 3 topic LDA terms still turn up. And it still seems that surgery is mostly about a single topic.
